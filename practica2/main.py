...
video = cv2.VideoCapture("people_walking2.mp4")
...
while True:
    ...
    for body in bodies:
        ...
        if 20 < width < 65 and 35 < height < 115:
            ...
            if current_frame == 1:
                person_id = Person.create_person(center, x, y, width, height, template, current_frame)
            else:
                coincidence = Person.same_person(template, frame, center, x, y, current_frame)
                if coincidence is not None:
                    color = coincidence.color
                    person_id = coincidence.person_id
                else:
                    person_id = Person.create_person(center, x, y, width, height, template, current_frame)
            rectangle = cv2.rectangle(rectangle_frame, (x, y), (x + width, y + height), color, 2)
            cv2.putText(rectangle_frame, f"{person_id}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
